...
var sec, secretarium = sec = {
    ...
    scp: class {
        ...
        connect(url, userKey, knownTrustedKey, protocol = "pair1", aesGcm = false) {
            ...
            return new Promise((resolve, reject) => {
                ...
                .then(async serverIdentity => {
                    ...
                    self.aesctr = {
                        ...
                        cryptokey: await window.crypto.subtle.importKey("raw", key, { name: aesGcm ? "AES-GCM": "AES-CTR" }, false, ["encrypt", "decrypt"])
                    }
                    ...
                    let nonce = sec.utils.getRandomUint8Array(32),
                        ...
                        aesMode = aesGcm ? { name: "AES-GCM", iv: iv.subarray(0, 12), tagLength: 128 } : { name: "AES-CTR", counter: iv, length: 128 },
                        encryptedClientProofOfIdentity = await window.crypto.subtle.encrypt(aesMode, self.aesctr.cryptokey, clientProofOfIdentity);
                    return new Promise((resolve, reject) => {
                ...
                .then(async serverProofOfIdentityEncrypted => {
                    let ivOffset = serverProofOfIdentityEncrypted.subarray(0, 16),
                        iv = self.aesctr.iv.secIncrementBy(ivOffset),
                        aesMode = aesGcm ? { name: "AES-GCM", iv: iv.subarray(0, 12), tagLength: 128 } : { name: "AES-CTR", counter: iv, length: 128 },
                        serverProofOfIdentity = await window.crypto.subtle.decrypt(aesMode, self.aesctr.cryptokey, serverProofOfIdentityEncrypted.subarray(16)),
        ...
        async _onMessage(x) {
            ...
            let aesMode = this.aesGcm ? { name: "AES-GCM", iv: iv.subarray(0, 12), tagLength: 128 } : { name: "AES-CTR", counter: iv, length: 128 },
                decrypted = await window.crypto.subtle.decrypt(aesMode, self.aesctr.cryptokey, x.subarray(16)),
        ...
        newTx(dcapp, command, requestId, args) {
            ...
            cmd.onResult = x => { (cbs["onResult"] = cbs["onResult"] || []).push(x); return cmd; }; // for chained tx + query
            cmd.send = () => { this.send(dcapp, command, requestId, args, cmd); return cmd; };
            return cmd;
        }
        ...
        async send(dcapp, command, requestId, args, cmd) {
            ...
            let query = JSON.stringify({ "dcapp": dcapp, "function": command, "requestId": requestId, args: args }),
                ...
                aesMode = this.aesGcm ? { name: "AES-GCM", iv: iv.subarray(0, 12), tagLength: 128 } : { name: "AES-CTR", counter: iv, length: 128 },
                self = this;

            console.debug("sending:" + query);

            return window.crypto.subtle.encrypt(aesMode, this.aesctr.cryptokey, data)
                .then(msg => {
    ...
    utils: (function() {
        ...
        return {
            ...
            isVErified(o, name) {
                return o && o[name] && o[name].attestations && o[name].attestations.Secretarium && o[name].attestations.Secretarium.verified;
            }
        }
    })()
}